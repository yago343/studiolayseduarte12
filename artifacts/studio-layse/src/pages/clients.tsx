...
import { useListClients, useGetClient } from "@workspace/api-client-react";
import { Card, CardContent } from "@/components/ui/card";
import { Button } from "@/components/ui/button";
import { Input } from "@/components/ui/input";
import { Dialog, DialogContent, DialogHeader, DialogTitle } from "@/components/ui/dialog";
import { Users, Search, History, Calendar, Phone } from "lucide-react";
      ...
      <div className="space-y-6 p-4">
        <div className="flex items-center gap-4 bg-primary/5 p-4 rounded-2xl">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground flex items-center gap-1 mt-1"><Phone className="w-3 h-3" /> {client.phone || "Sem telefone"}</p>
          </div>
        </div>
        
        <div>
          <h4 className="font-bold flex items-center gap-2 mb-3 text-foreground"><History className="w-4 h-4 text-primary" /> Histórico de Agendamentos</h4>
              ...
              {client.appointments.map(apt => (
                <div key={apt.id} className="p-3 bg-muted/30 rounded-xl border border-border/50 flex justify-between items-center">
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground mt-0.5"><Calendar className="w-3 h-3 inline mr-1" />{format(new Date(apt.date), 'dd/MM/yyyy')}</p>
                  </div>
                  <span className="text-xs font-bold px-2 py-1 bg-background rounded-md shadow-sm">R$ {apt.servicePrice}</span>
                </div>
              ))}
            </div>
          )}
  ...
  return (
    <div className="space-y-6">
            ...
            filteredClients?.map(client => (
              ...
              >
                ...
                <div className="text-right">
                  ...
                  {client.lastAppointment && <p className="text-xs text-muted-foreground mt-0.5">Última: {format(new Date(client.lastAppointment), 'dd/MM/yy')}</p>}
                </div>
              </div>
            ))
          )}
      ...
      <Dialog open={!!selectedClientId} onOpenChange={() => setSelectedClientId(null)}>
        <DialogContent className="rounded-3xl border-border p-0 overflow-hidden sm:max-w-md">
